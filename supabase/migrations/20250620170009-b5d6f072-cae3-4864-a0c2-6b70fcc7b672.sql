-- Fix unrealistic pallets per hour calculations by adding minimum time thresholds
-- This prevents edge cases where trucks are completed in seconds from skewing KPI metrics
...
BEGIN
  ...
  -- Insert refreshed metrics with FIXED pallets per hour calculation
  ...
  SELECT 
    ...
    -- FIXED: Average processing hours with minimum threshold of 0.1 hours (6 minutes)
    CASE 
      ...
      THEN AVG(CASE 
        ...
        THEN GREATEST(0.1, public.safe_calculate_processing_hours(t.started_at, t.completed_at)) / GREATEST(1, (
          SELECT COUNT(*) FROM (
            SELECT t.handled_by_user_id as user_id WHERE t.handled_by_user_id IS NOT NULL
            UNION
            SELECT th2.handler_user_id as user_id FROM truck_handlers th2 WHERE th2.truck_id = t.id
    ...
    -- FIXED: Pallets per hour with minimum processing time threshold (0.1 hours = 6 minutes)
    CASE 
      WHEN SUM(CASE 
        WHEN (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id)
        THEN GREATEST(0.1, public.safe_calculate_processing_hours(t.started_at, t.completed_at)) / GREATEST(1, (
...
-- Refresh today's KPI metrics with the corrected calculation
