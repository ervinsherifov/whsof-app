-- Fix the KPI function to properly include ALL handlers (including helpers)
...
BEGIN
  ...
  -- Insert refreshed metrics including ALL handlers (primary + helpers)
  ...
  SELECT 
    ...
    COUNT(DISTINCT CASE WHEN (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id) THEN t.id END) as total_trucks_handled,
    COUNT(DISTINCT CASE WHEN (t.status = 'COMPLETED' OR t.status = 'DONE') AND (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id) THEN t.id END) as completed_trucks,
    AVG(CASE 
      WHEN t.completed_at IS NOT NULL AND t.started_at IS NOT NULL AND (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id)
    ...
    -- Calculate pallets: divide by total handlers for each truck the user worked on
    SUM(CASE 
...
-- Refresh the KPI data to include helpers
