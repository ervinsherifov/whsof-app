-- Fix KPI calculations to follow proper business logic
...
BEGIN
  ...
  -- Insert refreshed metrics with corrected business logic
  ...
  SELECT 
    ...
    -- CORRECTED: Average processing hours per truck (weighted by user's involvement)
    CASE 
    ...
    -- CORRECTED: Average pallets per truck for this user
    CASE 
      WHEN COUNT(CASE WHEN (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id) THEN 1 END) > 0
      THEN AVG(CASE 
        WHEN (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id) THEN 
    ...
    -- CORRECTED: Pallets per hour = Total pallets handled / Total processing time
    CASE 
      WHEN SUM(CASE 
        WHEN (t.handled_by_user_id = users.user_id OR th.handler_user_id = users.user_id)
