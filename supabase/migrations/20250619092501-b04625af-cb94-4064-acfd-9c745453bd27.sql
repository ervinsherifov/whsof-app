-- Fix the handle_truck_arrival function to ensure actual_arrival_time is always set
...
DECLARE
  ...
  arrival_type TEXT;
  v_actual_arrival_date DATE := (now() AT TIME ZONE 'UTC+3')::DATE;
  v_actual_arrival_time TIME := (now() AT TIME ZONE 'UTC+3')::TIME;
BEGIN
  -- Get truck details
  ...
  IF was_late THEN
    INSERT INTO public.truck_notifications (
      ...
      ', Actual: ' || v_actual_arrival_date || ' ' || to_char(v_actual_arrival_time, 'HH24:MI'),
      'WARNING'
    );
  END IF;
