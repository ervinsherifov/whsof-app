-- Update handle_truck_arrival function to use GMT+3 timezone
...
DECLARE
  ...
  v_actual_arrival_time TIME;
  gmt3_now TIMESTAMP WITH TIME ZONE;
BEGIN
  -- Get current time in GMT+3
  gmt3_now := now() AT TIME ZONE 'UTC' AT TIME ZONE '+03:00';
  v_actual_arrival_date := gmt3_now::DATE;
  v_actual_arrival_time := gmt3_now::TIME;
  ...
  IF was_late THEN
    INSERT INTO public.truck_notifications (
      ...
      truck_record.arrival_date || ' at ' || truck_record.arrival_time || 
      ', Actual: ' || v_actual_arrival_date || ' at ' || v_actual_arrival_time || ' (GMT+3)',
      'WARNING'
    );
  END IF;
