-- Update handle_truck_arrival function to use Eastern European Summer Time (Bulgaria timezone)
...
DECLARE
  ...
  arrival_type TEXT;
  -- Use Eastern European Summer Time (Bulgaria timezone)
  bulgaria_now TIMESTAMP WITH TIME ZONE := now() AT TIME ZONE 'Europe/Sofia';
  v_actual_arrival_date DATE := (now() AT TIME ZONE 'Europe/Sofia')::DATE;
  v_actual_arrival_time TIME := (now() AT TIME ZONE 'Europe/Sofia')::TIME;
BEGIN
  -- Get truck details
  ...
  -- Update truck status with actual arrival date and time (always set both)
  ...
  INSERT INTO public.truck_status_history (
    ...
    jsonb_build_object(
      ...
      'actual_time', to_char(v_actual_arrival_time, 'HH24:MI'),
      'was_late', was_late,
  ...
  IF was_late THEN
    INSERT INTO public.truck_notifications (
      ...
      truck_record.arrival_date || ' ' || to_char(truck_record.arrival_time, 'HH24:MI') || 
      ', Actual: ' || v_actual_arrival_date || ' ' || to_char(v_actual_arrival_time, 'HH24:MI') || ' (Bulgaria time)',
      'WARNING'
    );
  END IF;
